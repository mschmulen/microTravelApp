...
@implementation travelAppMobileObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_travelappmobile);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any travelAppMobile patches in JS before a context is loaded... nice for 
...
{
	// create travelAppMobile global object
	NSString *basePath = (url==nil) ? [[NSBundle mainBundle] resourcePath] : [[url path] stringByDeletingLastPathComponent];
	_travelappmobile = [[travelAppMobileObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_travelappmobile];
	
	NSString *_travelappmobileNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithUTF8CString([_travelappmobileNS UTF8String]);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_travelappmobile valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.travelappmobile.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
